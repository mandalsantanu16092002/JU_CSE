...
class Server():
    ...
    def process_query(self):
        ...
        while True:
            res = input('Do you want to close the server yes or no : ')
            if res == 'yes':
                return

            while True:
                res = input('Do you want to change the authorization to any client yes or no : ')
                if res == 'yes':
                    client_no, auth = input(
                        'Enter client number and role (manager/guest): ').split()
                    if auth == 'manager' or auth == 'guest':
						#the role “manager” can access other’s key-value stores
            ...
            sock.bind((self.serverhost, self.serverport)) #address associate/bind
            sock.listen(1) 
            conn = sock.accept() #connection estublished
                ...
                if query[i] == 'get':
                    if i+1 < qlen:
                        if query[i+1] in self.key_store[client_no]:
                            response.append(
                                self.key_store[client_no][query[i+1]])
                            print('GET REQUEST: Sending the Value of \'' +
                                  query[i+1]+'\' as \''+self.key_store[client_no][query[i+1]]+'\' to client-'+str(client_no))
                        else:
                            if self.auth_dict[client_no] == 'manager':
                                print(
                                    query[i+1], 'could not be found in self key-value store')
                                print(
                                    'Since the client is a manager, Finding it in other\'s key-value store')
                                found = 0
                                for j in range(1, self.max_client+1):
                                    if query[i+1] in self.key_store[j]:
                                        found = 1
                                        response.append(
                                            self.key_store[j][query[i+1]])
                                        print('GET REQUEST: Sending the Value of \'' +
                                              query[i+1]+'\' as \''+self.key_store[j][query[i+1]]+'\' to client-'+str(client_no))
                                        break
                                if found == 0:
                                    response.append("<blank>")
                                    print('GET REQUEST: Sending the Value of \'' +
                                          query[i+1]+'\' as \''+'<blank>'+'\' to client-'+str(client_no))
                            else:
                                response.append("<blank>")
                                print('GET REQUEST: Sending the Value of \'' +
                                      query[i+1]+'\' as \''+'<blank>'+'\' to client-'+str(client_no))
                    ...
                    if i+2 < qlen:
                        ...
                        print('PUT REQUEST: Value of \''+key+'\' as \''+value +
